...
'''调整削峰时段'''
peak = list()
peak.extend(range(72,76))
## peak.extend(range(76,87))
powerLimit = 7000
'''save the parameters'''
...
load_data = pd.read_excel('output.xlsx')
load_before = load_data['电网购电功率'].tolist()
load_ref = load_data['期望功率'].tolist()

'''Construct the OptimalDispatch linear programming model'''
optimalDispatch = LpProblem('demand_response',LpMinimize)
...
for i in range(96):
    '''of Demand Response'''
    aux_var.append(LpVariable('aux_var_'+str(i),lowBound=0))
'''Define the objectives'''
optimalDispatch += lambda2 * lpSum(om_es1) \
    + lambda2 * lpSum(om_absc1) \
    + lambda2 * lpSum(om_bol1) \
    + lambda2 * lpSum(om_cs1) \
    + lambda2 * lpSum(om_ac1) \
    + lambda2 * lpSum(om_gte1) \
    + lambda2 * lpSum(z_pwut) \
    + lambda2 * lpSum(aux_pwst) \
    + lambda2 * lpSum(power_gte1)*(ut.gas_price*0.25/gt1.efficiency) \
    + lambda2 * lpSum(power_bol1)*(ut.gas_price*0.25/bol1.efficiency)\
    + lambda2 * + lpSum(high_heat)*(ut.steam_price*0.25)\
    + lambda1 * lpSum([aux_var[i] for i in peak])
    #+ lambda2 * lpSum(om_hs1) \
'''Define the constraints'''
optimalDispatch = define_Constraints(optimalDispatch,pv1,es1,absc1,bol1,cs1,ac1,gt1,ut,inv)
## 非削峰时段的关口功率限制
for i in range(96):
    if i not in peak:
        optimalDispatch += power_utility[i] <= powerLimit
    if i in peak:
        optimalDispatch += power_utility[i] >= load_ref[i]
    ##unsatisfied power reduction
    optimalDispatch += aux_var[i] >= (power_utility[i] - load_ref[i])
    optimalDispatch += aux_var[i] >= - (power_utility[i] - load_ref[i])
...
om_cost = pd.Series([x.varValue for x in om_ac1])+pd.Series([x.varValue for x in om_cs1])+pd.Series([x.varValue for x in om_absc1])+ \
          pd.Series([x.varValue for x in om_bol1])+pd.Series([x.varValue for x in om_es1])+pd.Series([x.varValue for x in om_gte1])\
          #pd.Series([x.varValue for x in om_hs1])
...
df['期望功率'] = pd.Series(load_ref)
df['削减前功率'] = pd.Series(load_before)
df['费用'] = pd.Series([x.varValue for x in hourly_cost])
df['电费'] = electricity_cost
df['热费'] = heat_cost
df['气费'] = fuel_cost
df['运维费用'] = om_cost
df['折旧费用'] = dep_cost
df.to_excel('responsed.xlsx')
